...
export function registerCardHandlers(bot: Bot, config: CardsConfig): void {
  const { token, githubOwner, githubRepo, githubBranch, githubToken, githubBasePath, plantnetApiKey, isAuthorized } = config;

  const lastImages = new Map<number, LastImage>();
  const pendingPlants = new Map<number, PendingPlant>();
